...
class DatabricksS3StreamCopierV2(
    ...
    private val s3Config: S3DestinationConfig =
        databricksConfig.storageConfig?.s3DestinationConfigOrThrow as S3DestinationConfig
    private val parquetWriter: S3ParquetWriter = writerFactory.create(
        getStagingS3DestinationConfig(s3Config, stagingFolder),
        s3Client ?: throw IllegalStateException("S3 client is required for S3 stream copier"),
        configuredStream,
        uploadTime,
    ) as S3ParquetWriter
    private val selectColumns by lazy {
        parquetWriter.schema?.let { getSelectColumns(it) }
    }
